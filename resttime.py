...
class RestTime:
        ...
        if sys.platform == "linux":
            ...
            try:
                ...
                if prop and prop.value:
                    self.log(
                        f"Encontrado ID de Ventana: {hex(self.window_id)}")
    ...
    def create_layout(self):
        ...
        self._tiempo_frame.grid(row=0, column=0, sticky=(tk.E, tk.W, tk.N,
                                                         tk.S))

        self._horas1_sv = tk.StringVar()
        self._horas1_sv.set(0)
        self._horas1 = ttk.Entry(self._tiempo_frame,
                                 textvariable=self._horas1_sv, width=1,
                                 font=('Arial', 150))
        ...
        self._scale_horas1 = ttk.Scale(self._tiempo_frame, orient=tk.VERTICAL,
                                       command=self._horas1_update, from_=9,
                                       to=0)
        self._scale_horas1.grid(row=0, column=2, sticky=(tk.S, tk.N))

        self._horas2_sv = tk.StringVar()
        self._horas2_sv.set(0)
        self._horas2 = ttk.Entry(self._tiempo_frame,
                                 textvariable=self._horas2_sv, width=1,
                                 font=('Arial', 150))
        ...
        self._scale_horas2 = ttk.Scale(self._tiempo_frame, orient=tk.VERTICAL,
                                       command=self._horas2_update,
                                       from_=9, to=0)
        self._scale_horas2.grid(row=0, column=4, sticky=(tk.S, tk.N))

        self._colon_horas = ttk.Label(self._tiempo_frame, width=0,
                                      font=('Arial', 150), text=":")
        self._colon_horas.grid(row=0, column=5, sticky=(tk.S, tk.N))

        self._minutos1_sv = tk.StringVar()
        self._minutos1_sv.set(0)
        self._minutos1 = ttk.Entry(self._tiempo_frame,
                                   textvariable=self._minutos1_sv, width=1,
                                   font=('Arial', 150))
        ...
        self._scale_minutos1 = ttk.Scale(self._tiempo_frame,
                                         orient=tk.VERTICAL,
                                         command=self._minutos1_update,
                                         from_=5, to=0)
        self._scale_minutos1.grid(row=0, column=7, sticky=(tk.S, tk.N))

        self._minutos2_sv = tk.StringVar()
        self._minutos2_sv.set(0)
        self._minutos2 = ttk.Entry(self._tiempo_frame,
                                   textvariable=self._minutos2_sv, width=1,
                                   font=('Arial', 150))
        ...
        self._scale_minutos2 = ttk.Scale(self._tiempo_frame,
                                         orient=tk.VERTICAL,
                                         command=self._minutos2_update,
                                         from_=9, to=0)
        self._scale_minutos2.grid(row=0, column=9, sticky=(tk.S, tk.N))

        self._colon_minutos = ttk.Label(self._tiempo_frame, width=0,
                                        font=('Arial', 150), text=":")
        self._colon_minutos.grid(row=0, column=10, sticky=(tk.S, tk.N))

        self._segundos1_sv = tk.StringVar()
        self._segundos1_sv.set(0)
        self._segundos1 = ttk.Entry(self._tiempo_frame,
                                    textvariable=self._segundos1_sv,
                                    width=1, font=('Arial', 150))
        ...
        self._scale_segundos1 = ttk.Scale(self._tiempo_frame,
                                          orient=tk.VERTICAL,
                                          command=self._segundos1_update,
                                          from_=5, to=0)
        self._scale_segundos1.grid(row=0, column=12, sticky=(tk.S, tk.N))

        self._segundos2_sv = tk.StringVar()
        self._segundos2_sv.set(0)
        self._segundos2 = ttk.Entry(self._tiempo_frame,
                                    textvariable=self._segundos2_sv, width=1,
                                    font=('Arial', 150))
        ...
        self._scale_segundos2 = ttk.Scale(self._tiempo_frame,
                                          orient=tk.VERTICAL,
                                          command=self._segundos2_update,
                                          from_=9, to=0)
        self._scale_segundos2.grid(row=0, column=14, sticky=(tk.S, tk.N))

        self._start_btn = ttk.Button(self._tiempo_frame, text="Start",
                                     underline=0, command=self._start_timer,
                                     padding="5 5 5 5")
        self._root.bind('<Alt-i>', lambda event: self._start_timer())
        self._start_btn.grid(row=1, column=13, sticky=(tk.E, tk.W), padx=10,
                             pady=10)
        self._start_btn.rowconfigure(1, weight=1)

        self._stop_btn = ttk.Button(self._tiempo_frame, text="Stop",
                                    underline=0, command=self._stop_timer,
                                    padding="5 5 5 5")
        self._root.bind('<Alt-i>', lambda event: self._stop_timer())
        self._stop_btn.grid(row=1, column=11, sticky=(tk.E, tk.W), padx=10,
                            pady=10)
        self._stop_btn.rowconfigure(1, weight=1)

        self._log_frame = ttk.LabelFrame(self._mainframe, text="Log",
                                         padding="10")
        self._log_frame.grid(row=2, column=0, pady=10, sticky=(tk.E, tk.W,
                                                               tk.N, tk.S))

        self._text_log = tk.Text(self._log_frame, height=6,
                                 font=('Courier', 10))
        self._text_log.grid(row=0, column=0, pady=5, sticky=(tk.E, tk.W, tk.N,
                                                             tk.S))
        self._scrollbar_tl = ttk.Scrollbar(self._log_frame, orient=tk.VERTICAL,
    ...
    def _start_timer(self):
        if not self.timer_activo:
            self.timer_contador = int(self._horas1_sv.get()) * 60 * 60 * 10 +\
                int(self._horas2_sv.get()) * 60 * 60 +\
                int(self._minutos1_sv.get()) * 60 * 10 +\
                int(self._minutos2_sv.get()) * 60 +\
                int(self._segundos1_sv.get()) * 10 +\
                int(self._segundos2_sv.get())
            if self.timer_contador > 0:
                ...
                self.update_counter()
                self._root.iconify()
                # Windows necesita perder el foco para que funcione el aviso.
                if sys.platform == "win32":
    ...
    def update_counter(self):
        if self.timer_activo:
            ...
            min1 = int((self.timer_contador - (hor1 * 36000) -
                        (hor2 * 3600)) // 600)
            min2 = int((self.timer_contador - (hor1 * 36000) - (hor2 * 3600) -
                        (min1 * 600)) // 60)
            seg1 = int((self.timer_contador - (hor1 * 36000) - (hor2 * 3600) -
                        (min1 * 600) - (min2 * 60)) // 10)
    ...
    def muestra_parpadeo_linux(self):
        ...
        atom_demands_attention =\
            self.d.intern_atom('_NET_WM_STATE_DEMANDS_ATTENTION')
        ...
        root = self.d.screen().root
        root.send_event(event_data, event_mask=X.SubstructureNotifyMask |
